...
class CucumberAdapter {
    ...
    async run () {
        ...
        global.defineSupportCode = defineSupportCode
        ...
        const hookListener = new Formatter()
        const hookRunner = new HookRunner(hookListener, this.config)

        const reporterListener = new Formatter()
    ...
    wrapSteps () {
        defineSupportCode(({setDefinitionFunctionWrapper}) => {
            setDefinitionFunctionWrapper((fn, options) => {
                ...
                let wrappedFunction = isAsync(fn) || this.config.sync === false
