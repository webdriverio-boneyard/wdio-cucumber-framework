...
class CucumberReporter {
    ...
    handleStepResultEvent (event, callback) {
         ...
         */
        if (!step.getName() && stepResult.getStatus() !== Status.FAILED) {
            return process.nextTick(callback)
        }

        if (stepResult.getStatus() === Status.UNDEFINED) {
        ...
        } else if (stepResult.getStatus() === Status.FAILED) {
            /**
            ...
            this.failedCount++
        } else if (stepResult.getStatus() === Status.AMBIGUOUS && this.options.failAmbiguousDefinitions) {
            e = 'fail'
            this.failedCount++
            error = {
